...
const isAccessTokenValid = async (page) => {
	const cookies = await page.cookies();
	const tokenCookie = await cookies.find((cookie) => cookie.name === "opfinproduction");
...
const getCsrfToken = async (page) => {
	...
	const csrfTokens = await page.evaluate(() => Array.from(document.getElementsByName("csrf-token"), (e) => e.value));
